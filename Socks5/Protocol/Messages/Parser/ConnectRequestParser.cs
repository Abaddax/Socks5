...
namespace Abaddax.Socks5.Protocol.Messages.Parser
{
    internal partial class Socks5Parser :
        ...
        IStreamParser<ConnectRequest>
    {

        #region IBinaryParser<ConnectRequest>
        ConnectRequest IBinaryParser<ConnectRequest>.Read(ReadOnlySpan<byte> packet)
        {
            using var ms = new MemoryStream();
            ms.Write(packet);
            return ((IStreamParser<ConnectRequest>)this).ReadAsync(ms, default).Result;
        }
        int IBinaryParser<ConnectRequest>.GetMessageSize(ConnectRequest message)
        {
        ...
        int IBinaryParser<ConnectRequest>.Write(ConnectRequest message, Span<byte> destination)
        {
        ...
        async Task<ConnectRequest> IStreamParser<ConnectRequest>.ReadAsync(Stream stream, CancellationToken token)
        {
        ...
        async Task IStreamParser<ConnectRequest>.WriteAsync(Stream stream, ConnectRequest message, CancellationToken token)
        {
            var size = ((IBinaryParser<ConnectRequest>)this).GetMessageSize(message);
